  ...
  <div class="form">
    ...
    <div>
      Dislike: <span>
        ...
        <label for="dislike">{{ category.dislike ? 'Enabled' : 'Disabled' }}</label>
      </span> 
    </div>
    <br>
    <div>
...
export default class CategorySettings extends Vue {
  ...
  async onSave() {
    try {
      await this.col.doc(this.category.id).set(proxy(this.category), {merge: true});
      alert("Settings for " + this.category.id + " category successfully updated");
    } catch (e) {
      alert(e);
    }
  }
